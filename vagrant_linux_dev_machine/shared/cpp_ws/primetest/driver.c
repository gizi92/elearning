...
static bool convert_arg(const char* arg, unsigned long long* val) {
  ...
  // strtoll retirns and in-band error indicator; clear errno before the call.
  errno = 0;
...
static unsigned long long* convert_command_line_args(
  int argc,
  const char *argv[],
  size_t* num_args) {
  *num_args = 0;
  ...
  if (argc <= 1) {
    ...
    // origram being executed).
    print_help();
    return NULL;
  }

  // We know the maximum number of arguments the user could have passed,
  // so allocate an arrya large enough to hold all the elements. Substract
  // one for th program name istelf. If the allocation fails, treat it as
...
int main(int argc,const char *argv[]) {
  ...
  for(size_t i = 0; i < num_args; ++i) {
    printf("%llu is %s.\n", vals[i],
      is_prime(vals[i], 10) ? "probably prime" : "not prime");
  }
